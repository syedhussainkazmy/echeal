...
import { FileText, Download, Calendar, Users, ClipboardList, Loader2, Printer } from 'lucide-react';
        ...
        try {
            ...
            switch (reportType) {
                ...
                case 'patient-summary':
                    endpoint = '/reporting/patient/summary';
                    break;
                case 'admin-analytics':
                    endpoint = `/analytics/admin${queryString ? `?${queryString}` : ''}`;
                    break;
                default:
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Generate and export practice reports</p>
                </div>
            </div>
            ...
            <div className="bg-white rounded-md border border-gray-200 p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {[
                        ...
                        { value: 'patient-summary', label: 'Patient Summary', icon: Users },
                        { value: 'admin-analytics', label: 'Analytics', icon: FileText },
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="flex items-end">
                        <button
                            ...
                            disabled={loading}
                            className="w-full flex items-center justify-center gap-2 bg-teal-600 text-white py-2 px-4 rounded-md text-sm font-semibold hover:bg-teal-700 transition-colors disabled:opacity-60"
                        >
                            {loading ? (
                                <>
                ...
                <div className="bg-white rounded-md border border-gray-200 overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                        <h2 className="text-base font-semibold text-gray-800">
                            Report: {reportType.charAt(0).toUpperCase() + reportType.slice(1).replace('-', ' ')}
                        </h2>
                    ...
                    <div id="report-content" className="p-6">
                        ...
                        {reportData.period && (
                            <div className="mb-4 text-sm text-gray-500">
                                Period: {reportData.period.start ? format(new Date(reportData.period.start), 'MMM d, yyyy') : 'All'} - {reportData.period.end ? format(new Date(reportData.period.end), 'MMM d, yyyy') : 'All'}
                            </div>
                        )}
                        ...
                        {reportData.userStats && (
                            <div className="mb-6">
                                <h3 className="text-sm font-semibold text-gray-700 mb-2">User Distribution</h3>
                                <div className="grid grid-cols-3 gap-3">
                                    {Object.entries(reportData.userStats).map(([role, count]) => (
                                        <div key={role} className="bg-gray-50 rounded-md p-3">
                        ...
                        {reportData.appointments && (
                            <div>
                                ...
                                <div className="overflow-x-auto">
                                    ...
                                    {reportData.appointments.length > 20 && (
                                        <p className="text-sm text-gray-500 mt-2">Showing 20 of {reportData.appointments.length} appointments</p>
                                    )}
                                </div>
                            </div>
                        )}
                                        ...
                                        <tbody className="divide-y divide-gray-100">
                                            {reportData.prescriptions.slice(0, 20).map((presc: any, idx: number) => (
                                                <tr key={idx}>
                                                    ...
                                                    <td className="px-3 py-2">
                                                        <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${
                                                            presc.status === 'issued' ? 'bg-green-100 text-green-800' :
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-gray-50 rounded-md p-4">
                                        <p className="text-xs text-gray-500">Total Vitals</p>
                                        <p className="text-2xl font-bold text-gray-900">{reportData.vitalsSummary?.totalReadings || 0}</p>
                                    </div>
                                    <div className="bg-gray-50 rounded-md p-4">
                                        <p className="text-xs text-gray-500">Appointments</p>
                                        <p className="text-2xl font-bold text-gray-900">{reportData.appointments?.length || 0}</p>
                                    </div>
                                </div>
                            </div>
                        )}
